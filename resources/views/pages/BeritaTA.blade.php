...
@section('title')
    <title>Berita Acara Tugas Akhir - Admin</title>
@endsection

@section('page-title')

    <div class="page-title">
        <h4>Halaman Berita Acara Tugas Akhir</h4>
    </div>
...
<div class="panel panel-flat">
    <div class="panel-heading">
        <h5 class="panel-title">Data Berita Acara Tugas Akhir</h5>
    </div>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$item->nama_dospem1}} <br> {{$item->nama_dospem2}}</td>
                <td>{{$item->nama_dospeng1}} <br> {{$item->nama_dospeng2}}</td>
                <td>{{$item->nama_mhs}}</td>
                <td>{{ Carbon\Carbon::parse($item->tgl_ba)->format('d M Y') }}</td>
                <td>
                    <a href="{{url('upload/berita-acara/tugas-akhir/'.$item->file)}}" class="btn btn-primary btn-sm btn-icon" target="_blank"><i class="icon-pencil7"></i></a>
                    <a href="{{url('upload/berita-acara/'.$item->file)}}" class="btn btn-warning btn-sm btn-icon" target="_blank"><i class="icon-pencil7"></i></a>
                    <a href="{{route('berita-acara-ta.delete', $item->id)}}" class="btn btn-danger btn-sm btn-icon"><i class="icon-trash"></i></a>
...
<div id="AddBA" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST" action="{{route('berita-acara-ta.post')}}" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control nama-dosen" name="dospem_1" required></select>
                    </div>
                    <div class="form-group">
                        <label>Dosen Pembimbing 2</label>
                        <select class="form-control nama-dosen" name="dospem_2" required></select>
                    </div>
                    <div class="form-group">
                        <label>Dosen Penguji 1</label>
                        <select class="form-control nama-dosen" name="dospeng_1" required></select>
                    </div>
                    <div class="form-group">
                        <label>Dosen Penguji 2</label>
                        <select class="form-control nama-dosen" name="dospeng_2" required></select>
                    </div>
                    <div class="form-group">
